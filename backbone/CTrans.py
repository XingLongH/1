...
class Channel_Embeddings(nn.Module):
    ...
    def __init__(self,config, patchsize, img_size, in_channels):
        ...
        # print( patch_size,in_channels,n_patches)
        self.patch_embeddings = Conv2d(in_channels=in_channels,
                                       out_channels=in_channels,
                                       kernel_size=self.patch_size,
                                       stride=self.patch_size)
        self.position_embeddings = nn.Parameter(torch.zeros(1,self.n_patches, in_channels))
    ...
    def forward(self, x):
        ...
        # (1, H*W, hidden_dim) -> (1, hidden_dim, H*W)
        
        position_embeddings = nn.Parameter(self.position_embeddings.permute(0, 2, 1))
        
        # (1, hidden_dim, H*W) -> (1, hidden_dim, H, W)
        # print(position_embeddings.shape)
        position_embeddings =  nn.Parameter(position_embeddings.reshape(1, hidden_dim,
                                                                              self.img_size[0] // self.patch_size[0], self.img_size[1] // self.patch_size[1]))
       # Perform interpolation.
 
        position_embeddings = nn.Parameter(nn.functional.interpolate(
            position_embeddings,
            size=(W, H),
            mode='bilinear',
            align_corners=True))
        ...
        n_patches=position_embeddings.shape[2]*position_embeddings.shape[3]
        # print(self.position_embeddings.shape, n_patches)
        position_embeddings =  nn.Parameter(position_embeddings.reshape(1, hidden_dim, n_patches))

        # (1, hidden_dim, H*W) -> (1, new_seq_length, H*W)
        position_embeddings =  nn.Parameter(position_embeddings.permute(0, 2, 1))
...
class ChannelTransformer(nn.Module):
    ...
    def forward(self,en1,en2,en3,en4):
        ...
        x1 = nn.functional.interpolate(x1,size=(en1.size()[2], en1.size()[3]),mode='bilinear',align_corners=True)
        x2 = nn.functional.interpolate(x2,size=(en2.size()[2], en2.size()[3]),mode='bilinear',align_corners=True)
        x3 = nn.functional.interpolate(x3,size=(en3.size()[2], en3.size()[3]),mode='bilinear',align_corners=True)
        x4 = nn.functional.interpolate(x4,size=(en4.size()[2], en4.size()[3]),mode='bilinear',align_corners=True)
